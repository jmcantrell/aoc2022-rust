...
pub fn solve1(troop: &Parsed) -> anyhow::Result<Solution1> {
    Ok(monkey_business(troop, 20, |item| item / 3))
}

pub fn solve2(troop: &Parsed) -> anyhow::Result<Solution2> {
...
pub mod tests {
    ...
    fn solve1() -> anyhow::Result<()> {
        assert_eq!(super::solve1(&parse(INPUT)?)?, 10_605);
        Ok(())
    }

    #[test]
    fn solve2() -> anyhow::Result<()> {
        assert_eq!(super::solve2(&parse(INPUT)?)?, 2_713_310_158);
        Ok(())
    }
}
