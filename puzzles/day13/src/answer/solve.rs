...
pub fn solve1(packet_pairs: &Parsed) -> anyhow::Result<Solution1> {
    Ok(packet_pairs
        .iter()
        .enumerate()
...
pub fn solve2(packet_pairs: &Parsed) -> anyhow::Result<Solution2> {
...
pub mod tests {
    ...
    fn solve1() -> anyhow::Result<()> {
        assert_eq!(super::solve1(&parse(INPUT)?)?, 13);
        Ok(())
    }

    #[test]
    fn solve2() -> anyhow::Result<()> {
        assert_eq!(super::solve2(&parse(INPUT)?)?, 140);
        Ok(())
    }
}
