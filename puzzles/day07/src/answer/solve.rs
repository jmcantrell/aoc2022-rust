...
pub fn solve1(output: &Parsed) -> anyhow::Result<Solution1> {
...
pub fn solve2(output: &Parsed) -> anyhow::Result<Solution2> {
    let file_system = output.reconstruct_file_system()?;
...
pub mod tests {
    ...
    fn solve1() -> anyhow::Result<()> {
        assert_eq!(super::solve1(&parse(INPUT)?)?, 95437);
        Ok(())
    }

    #[test]
    fn solve2() -> anyhow::Result<()> {
        assert_eq!(super::solve2(&parse(INPUT)?)?, 24933642);
        Ok(())
    }
}
