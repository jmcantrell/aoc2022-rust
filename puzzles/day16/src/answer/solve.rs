...
pub fn solve1(graph: &Parsed) -> anyhow::Result<Solution1> {
    let seconds = 30;
...
pub fn solve2(graph: &Parsed) -> anyhow::Result<Solution2> {
    let seconds = 26;
    let mut max_released = 0;
    let all_valves: HashSet<_> = graph.flow_rates.keys().cloned().collect();
...
pub mod tests {
    ...
    fn solve1() -> anyhow::Result<()> {
        assert_eq!(super::solve1(&parse(INPUT)?)?, 1_651);
        Ok(())
    }

    #[test]
    fn solve2() -> anyhow::Result<()> {
        assert_eq!(super::solve2(&parse(INPUT)?)?, 1_707);
        Ok(())
    }
}
