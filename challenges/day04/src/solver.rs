...
impl Solver for Solver1 {
    ...
    fn solve(&self) -> Self::Solution {
        Ok(self.0.iter().filter(|pair| pair.has_redundancy()).count())
    }
}
...
impl Solver for Solver2 {
    ...
    fn solve(&self) -> Self::Solution {
        Ok(self.0.iter().filter(|pair| pair.has_overlap()).count())
    }
}
...
mod tests {
    ...
    fn solve1() -> anyhow::Result<()> {
        assert_eq!(Solver1(Parser1(INPUT).parse()?).solve()?, 2);
        Ok(())
    }

    #[test]
    fn solve2() -> anyhow::Result<()> {
        assert_eq!(Solver2(Parser2(INPUT).parse()?).solve()?, 4);
        Ok(())
    }
}
